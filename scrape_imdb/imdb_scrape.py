...
def collect_movie_dict(movie_id):
    page_json = scrape_index_page(movie_id)  # Retrieve the scraped data
    if page_json:
        try:
            page_json = json.loads(page_json)  # Load JSON if data is available
            movie = {
                "name": page_json.get("name", ""),
                "genre": page_json.get("genre", ""),
                "image": page_json.get("image", ""),
            ...
            print(f"Error decoding JSON for movie_id: {movie_id}")
            return None
    else:
        return None
